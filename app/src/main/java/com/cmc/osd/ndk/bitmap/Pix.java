...
/**
 * Java representation of a native  PIX object.
 *
 */
public class Pix {
    static {
    ...
    public byte[] getLineData(int line){
    	if (line >= getHeight())
            throw new IllegalArgumentException("Height must be between 0 and " + Integer.toString(getHeight()));
    	
    	int wpl = (nativeGetWidth(mNativePix)*nativeGetDepth(mNativePix)+31)/32;
    	int lineSize = 4 * wpl;
    	
    	byte[] buffer = new byte [lineSize];
    	
        if (!nativeGetLineData(mNativePix, line, buffer)) {
            throw new RuntimeException("native getLineData failed");
        }
    ...
    public int getDataSize(){
    	return nativeGetDataSize(mNativePix);
    }
    ...
    private static native boolean nativeGetLineData(int nativePix, int line, byte[] data);
